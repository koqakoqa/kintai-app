...
function calcWorkMins(checkIn, checkOut, breakMins) {
  if (!checkIn || !checkOut) return 0;
  const ip = checkIn.slice(0,5).split(":"); const op = checkOut.slice(0,5).split(":");
  return (parseInt(op[0])*60+parseInt(op[1])) - (parseInt(ip[0])*60+parseInt(ip[1])) - (breakMins||0);
}
          ...
          <Card style={{ marginBottom: 20 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 16, marginBottom: 16 }}>
              {[{ label: "出勤", val: today && today.check_in ? today.check_in.slice(0,5) : "--:--", color: GREEN },
                { label: "休憩", val: today ? (today.break_mins||0)+"分" : "--", color: YELLOW },
...
function AttendanceEditModal(props) {
  ...
  async function save() {
    ...
    try {
      ...
      await api.upsertAttendance(empId, date, data);
      onSaved();
    } catch (e) { setError(e.message); }
    setSaving(false);
  }
  ...
  return (
    <Modal open={true} title={(rec ? "勤怠を編集" : "勤怠を追加") + " - " + date}>
...
function HistoryView(props) {
  ...
  function openAdd() {
    ...
    setEditModal({ rec: null, empId: isAdmin ? filterEmpId : currentEmp.id, date: d });
  }
  ...
  return (
    <div>
      {editModal && (
        <AttendanceEditModal rec={editModal.rec} empId={editModal.empId} date={editModal.date}
      ...
      {loading ? <Spinner /> : (
        <div style={{ background: CARD, border: "1px solid " + BORDER, borderRadius: 8, overflow: "hidden", position: "relative" }}>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            <thead>
              <tr style={{ background: PANEL }}>
                {["氏名","本日の状況","有給残日数","今月の稼働時間"].map(function(h) {
    ...
    <div style={{ background: BG, minHeight: "100vh", color: TEXT, fontFamily: "'Inter','Noto Sans JP',sans-serif" }}>
      ...
      <div style={{ background: PANEL, borderBottom: "1px solid " + BORDER, padding: "0 28px", display: "flex", alignItems: "center", justifyContent: "space-between", height: 60, position: "sticky", top: 0, zIndex: 100, boxShadow: "0 2px 20px rgba(0,0,0,0.4)" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <span style={{ color: TEXTSUB, fontSize: 13 }}>{loggedInEmp.name}</span>
